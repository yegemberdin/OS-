...
int main()
{
 ...
 for(i = 0; i < np; i++)
 {
  ...
  if(flag == 0) 
  {
   ...
   {
    /*here we will calculate distances to future occurance for every frame containing page*/
    for(j = 0; j < nf; j++)
    {
     for(temp = i + 1; temp < np; temp++) 
     {
      distances[j] = 0; 
      if (frames[j] == pages[temp]) // If that page occurs in the future we will store this didtance in the following array
      {
       distances[j] = temp-i; 
       break;
      }
    ...
    for(j = 0; j < nf; j++) // here we just take case when it is not occured in the future, if so we will simply set this frame index to variable index
    {
     if(distances[j] == 0)
     {           
      index = j; 
   ...
   //in the last case we will take candidate that is having maximum occurance distance in  the future, for this we just compare distances and take maximum
   if(found == 0)
   {
    maxdist= distances[0];
    index = 0;
